...
from stopro.analyzer.plot_drag3D import plot_each_drag3D
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--optimize", help="designate optimization or not", default=1
    )
    ...
    r_test = [r * scale for r in r_test]
    r_train = [r * scale for r in r_train]
    ...
    if optimize:
        ...
        theta = [init]
        norm_of_grads_list = [0.0]
    train_end_time = time.time()
    print(f"{train_end_time - train_start_time:.1f}s: training")
    print(
        f"cond. # after optimization: {jnp.linalg.cond(gp_model.trainingK_all(opt, r_train)):.1e}"
    )
    # predict
    ...
    # unpack values
    f_infer = [f / scale**2 for f in fs]
    std = [jnp.sqrt(jnp.diag(Σ)) / scale**2 for Σ in Σs]

    # change outside values to be zero
    sample = Drag3D(**params_setting)
    f_infer = [
        sample.change_outside_values_to_zero(r / scale, f)
        for r, f in zip(r_test, f_infer)
    ]
    ...
    # make plot
    plot_each_drag3D(contents, params_plot, params_prepare, params_main, lbls, vnames)
